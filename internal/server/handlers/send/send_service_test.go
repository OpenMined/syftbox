...
func TestUnmarshalResponse_WithBase64Body(t *testing.T) {
	// Test data with base64 encoded body (simplified version of the error case)
	responseData := `{
	...
	result, err := unmarshalResponse([]byte(responseData), false)
	assert.NoError(t, err)
	assert.NotNil(t, result)
	...
	assert.Equal(t, "alice@openmined.org", message["sender"])
	assert.Equal(t, "syft://alice@openmined.org/app_data/mit/rpc/search", message["url"])

	// The ID should be a UUID type, so we check it's not nil
	assert.NotNil(t, message["id"])

	// The status_code should be a SyftStatus type, so we check it's not nil
	assert.NotNil(t, message["status_code"])

	// Verify that the body was properly decoded and contains the expected content
	body, ok := message["body"].(map[string]interface{})
	assert.True(t, ok)
	assert.Equal(t, "0a5a918a-e1c7-43ef-9781-27e5277930db", body["id"])
	assert.Equal(t, "What is encrypted prompt  ? ", body["query"])
}
